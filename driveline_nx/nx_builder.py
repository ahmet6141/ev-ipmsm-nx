"""Siemens NX builder for the driveline. RUN INSIDE NX (headless) via run_journal:
...
journal just supplies the driveline blueprint and driveline body-naming / per-part
grouping. With NO blueprint argument the default EV driveline is built.

Export DEFAULTS to "step" (AP242, reliable); "parts" also writes each component
(Differential, Ring_Gear, Halfshaft_L/R, CV_Joints, Wheel_Hub_L/R) as its own STEP
for piece-by-piece production hand-off. See motor_nx/nx_builder.py for the per-call
NXOpen rationale; this file adds no new NXOpen calls.
"""
...
# driveline material-role DISPLAY NAMES (set via Body.SetName so the FEA / CAM can
# select bodies by role). Keyed by the component grouping below.
_DRIVELINE_ROLE_NAME = {
    "Differential": "DIFF_CARRIER",
    "Ring_Gear": "RING_GEAR",
    "Input": "INPUT_COUPLING",
    "Halfshaft_L": "HALFSHAFT_L",
    "Halfshaft_R": "HALFSHAFT_R",
    "CV_Joints": "CV_JOINT",
    "Wheel_Hub_L": "WHEEL_HUB_L",
    "Wheel_Hub_R": "WHEEL_HUB_R",
}


def _driveline_component_of(step_id):
    ...
    if base.startswith("diff_carrier") or base.startswith("side_gear"):
        return "Differential"
    if base.startswith("ring_gear"):
        return "Ring_Gear"
    if base.startswith("diff_input"):
        return "Input"
    if base.startswith("halfshaft_l"):
        return "Halfshaft_L"
    if base.startswith("halfshaft_r"):
        return "Halfshaft_R"
    if base.startswith("cv_"):
        return "CV_Joints"
    # wheel-hub bodies are tagged by a trailing side letter (..._l / ..._r)
    hub_prefixes = ("hub_bearing_", "encoder_ring_", "hub_flange_", "hub_pilot_",
                    "brake_pilot_", "hub_lug_", "hub_centre_nut_")
    if base.startswith(hub_prefixes):
        return "Wheel_Hub_L" if base.endswith("_l") else "Wheel_Hub_R"
    return None


def _default_blueprint():
    """Generate the default EV driveline blueprint in-process (no NXOpen needed for
    ...
    for _m in [m for m in list(sys.modules)
               if m == "driveline_nx" or m.startswith("driveline_nx.")]:
        del sys.modules[_m]
    from driveline_nx import blueprint as _bp
    from driveline_nx.params import DrivelineParams
    return _bp.generate(DrivelineParams())
...
def main():
    ...
    if out_prt is None:
        out_prt = os.path.join(os.path.dirname(os.path.abspath(__file__)), "driveline_out.prt")

    # Point the reused engine's role-name + component grouping at the driveline maps
    # (the methods read these as module globals at call time, so a clean swap).
    eng._ROLE_NAME = _DRIVELINE_ROLE_NAME
    eng._component_of = _driveline_component_of
    ...
    builder.log("=== driveline_nx build: %s ===" % blueprint.get("name", "driveline"))
